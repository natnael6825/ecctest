...
function TotalViewForUser() {
  ...
  // 1️⃣ fetch & compute full time‑series
  useEffect(() => {
    const fetchInteractions = async () => {
      try {
        const data = await fetchAllInteraction();
        // only user‑posted interactions
        const userData = data.filter(
          (i) => i.offer?.poster?.toLowerCase() === "user"
        );
  ...
  // 2️⃣ slice the data according to slider
  ...
  return (
    <div className="p-4">
      <h2>Total User Interactions Over Time</h2>
...
export default TotalViewForUser;
