...
function DailyUserPostsChart() {
  ...
  // 1️⃣ fetch & compute full date→count
  useEffect(() => {
    const fetchAndProcess = async () => {
      try {
        ...
        const userOnly = allPosts.filter(
          (post) => post.poster?.toLowerCase() === "user"
        );
        ...
        console.error(err);
        setError("Failed to load daily user posts");
      } finally {
        setLoading(false);
      }
    };
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: "User‑posted Products per Day",
        data: dataToShow,
        fill: false,
        borderColor: "green",
        tension: 0.4,
  ...
  return (
    <div className="p-4">
      <h2>Daily Products Posted by Users</h2>
...
export default DailyUserPostsChart;
