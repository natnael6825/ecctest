...
function DashboardStatusGrid() {
  ...
  useEffect(() => {
    const fetchAvgViewsPerPost = async () => {
      try {
        const posts = await getAllPost();

        const viewCounts = posts
          .map((post) => post.view_count)
          .filter((view) => typeof view === "number");

        const averageViews =
          viewCounts.length > 0
            ? viewCounts.reduce((sum, view) => sum + view, 0) /
              viewCounts.length
            : 0;

        setAvgUser(averageViews);
      } catch (error) {
        console.error("Error fetching posts for Avg View/Post:", error);
      }
    };

    fetchAvgViewsPerPost();
  }, []);
